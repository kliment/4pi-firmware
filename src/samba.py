...
parser.add_option("-b", "--binfile",
        ...
        type="string",
        dest="binfile",
        default="bin/Sprinter-4pi-at91sam3u4-flash.bin",
...
class samba:
    def __init__(self):
     ...
     Please close the ERASE jumper on the development kit and reboot the system (hit NRSTB button)!\n"""
            ...
            samba_cmd = "DISPLAY=:0 /home/kliment/eagle/smdsl/softpack/samba/sam-ba_cdc_cdc_linux/sam-ba %s %s %s"%(cmdOptions.port, cmdOptions.target,
                    self.f.name)
            if(cmdOptions.DEBUG):
